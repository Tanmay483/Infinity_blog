...
Category.findParentId = (iParentCatID, result) => {
    sql.query(
        `SELECT * FROM tbl_categories WHERE iParentCatID = ${iParentCatID}`,
        (err, res) => {
            if (err) {
                console.log('error: ', err);
                result(err, null);
...
Category.updateById = (cId, category, result) => {
    sql.query(
        ...
        [
            category.vCategoryName,
            category.vCategorySlug,
            category.vCategoryImage,
            category.iParentCatID,
            category.tCreatedDate,
            category.tUpdatedDate,
            cId,
        ],
        (err, res) => {
            if (err) {
            ...
            categories.forEach((category, index) => {
                sql.query(
                    ...
                    (err, resp) => {
                        ...
                        categories[index].subCategories = resp;

                        // Check if all subcategories have been assigned
                        counter++;
